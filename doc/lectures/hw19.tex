...
реализована конечнообъёмная схема решения нестационарной задачи о естественной
...
\begin{cppcode}
const double* u = uvp_.data() + 0;
const double* p = uvp_.data() + nu;
...
Для её решения испльзуется точный решатель \cvar{UmfMatrixSolver}.
...
Сначала точным (\cvar{UmfMatrixSolver}),
потом, если всё работает, можно перейти на итерационный
и сравнить время работы \cvar{AmgcMatrixSolver}.
