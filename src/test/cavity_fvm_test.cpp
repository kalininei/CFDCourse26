#include "cfd/debug/grid_manip.hpp"
...
const double rotation_angle = 1.0;
const Point rotation_center = Point{0.5, 0.5};

struct CavityFvmWorker {
    CavityFvmWorker(const IGrid& grid, double Re, double alpha_u, double alpha_p);
...
CavityFvmWorker::CavityFvmWorker(const IGrid& grid, double Re, double alpha_u, double alpha_p)
    : grid_(grid),
      Re_(Re),
      alpha_u_(alpha_u),
      alpha_p_(alpha_p),
    ...
    u_ = std::vector<double>(vec_size(), 0);
...
void CavityFvmWorker::gather_boundary_collocations() {
    ...
    for (size_t icolloc: collocations_.face_collocations) {
        ...
        // rotate back
        fc = rotate(fc, -rotation_angle, rotation_center);
        if (std::abs(fc.y - 1.0) < 1e-6) {
            bi.top.push_back(icolloc);
        } else {
            bi.nontop.push_back(icolloc);
        }
    }
}

void CavityFvmWorker::initialize_saver(std::string stem, size_t save_iter_step) {
    writer_.reset(new VtkUtils::TimeSeriesWriter(stem));
    writer_->set_iter_step(save_iter_step);
};

double CavityFvmWorker::to_next_iteration() {
...
std::vector<double> CavityFvmWorker::compute_un_star_face_rhie_chow(const std::vector<double>& u_star,
                                                                    const std::vector<double>& v_star) const {
...
void CavityFvmWorker::save_current_fields(size_t iter) const {
    if (writer_) {
...
std::vector<double> CavityFvmWorker::compute_dn_face(const std::vector<double>& x, const std::vector<double>& y) const {
...
CsrMatrix CavityFvmWorker::assemble_u_matrix() const {
...
void CavityFvmWorker::prepare_slae(const CsrMatrix& A) {
    mat_u_ = A;
    mat_v_ = A;
    ...
    // top bc: u=1, v=0
    Vector vel_top = rotate(Vector{1.0, 0}, rotation_angle);
    for (size_t icolloc: boundary_info_.top) {
        mat_u_.set_unit_row(icolloc);
        rhs_u_[icolloc] = vel_top.x;
        mat_v_.set_unit_row(icolloc);
        rhs_v_[icolloc] = vel_top.y;
    }
    // others
    for (size_t icolloc: boundary_info_.nontop) {
        mat_u_.set_unit_row(icolloc);
        rhs_u_[icolloc] = 0.0;
        mat_v_.set_unit_row(icolloc);
        rhs_v_[icolloc] = 0.0;
...
std::vector<double> CavityFvmWorker::compute_p(const std::vector<double>& un_face) const {
...
TEST_CASE("Cavity 2D, FVM-SIMPLE algorithm", "[cavity-fvm]") {
    std::cout << std::endl << "--- cfd_test [cavity-fvm] --- " << std::endl;
    ...
    // auto grid1 = RegularGrid2D(0, 1, 0, 1, 30, 30);
    auto grid1 = UnstructuredGrid2D::vtk_read(test_directory_file("tetragrid_500.vtk"));
    auto grid = dbg::rotate_grid(grid1, rotation_angle, rotation_center);
    CavityFvmWorker worker(grid, Re, alpha_u, alpha_p);
    worker.initialize_saver("cavity-fvm", 5);
