...
struct CylinerFvmWorker {
    CylinerFvmWorker(const IGrid& grid, double Re, double alpha_u, double alpha_p);
...
CylinerFvmWorker::CylinerFvmWorker(const IGrid& grid, double Re, double alpha_u, double alpha_p)
    : grid_(grid),
      Re_(Re),
      alpha_u_(alpha_u),
      alpha_p_(alpha_p),
...
void CylinerFvmWorker::gather_boundary_collocations() {
...
void CylinerFvmWorker::initialize_saver(std::string stem, size_t save_iter_step) {
    writer_.reset(new VtkUtils::TimeSeriesWriter(stem));
    writer_->set_iter_step(save_iter_step);
};

double CylinerFvmWorker::to_next_iteration() {
...
std::vector<double> CylinerFvmWorker::compute_un_star_face_rhie_chow(const std::vector<double>& u_star,
                                                                     const std::vector<double>& v_star) const {
...
void CylinerFvmWorker::save_current_fields(size_t iter) const {
    if (writer_) {
...
std::vector<double> CylinerFvmWorker::compute_dn_face(const std::vector<double>& x,
...
CsrMatrix CylinerFvmWorker::assemble_u_matrix() const {
...
void CylinerFvmWorker::prepare_slae(const CsrMatrix& A) {
    mat_u_ = A;
    mat_v_ = A;
...
std::vector<double> CylinerFvmWorker::compute_p(const std::vector<double>& un_face) const {
...
TEST_CASE("Stationar cyliner 2D, FVM-SIMPLE algorithm", "[stationar-cylinder-fvm]") {
    std::cout << std::endl << "--- cfd_test [stationar-cylinder-fvm] --- " << std::endl;
    ...
    CylinerFvmWorker worker(grid, Re, alpha_u, alpha_p);
    worker.initialize_saver("cylinder-fvm", 1);
