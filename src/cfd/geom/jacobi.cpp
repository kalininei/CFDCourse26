...
void cfd::fill_jacobi_modj(JacobiMatrix& jac) {
    jac.modj = jac.j11 * (jac.j22 * jac.j33 - jac.j23 * jac.j32) - jac.j12 * (jac.j21 * jac.j33 - jac.j23 * jac.j31) +
...
Vector cfd::gradient_to_parametric_1d(const JacobiMatrix& jac, Vector grad_x) {
    return {jac.j11 * grad_x.x};
}

Vector cfd::gradient_to_parametric_2d(const JacobiMatrix& jac, Vector grad_x) {
    return {jac.j11 * grad_x.x + jac.j21 * grad_x.y, jac.j12 * grad_x.x + jac.j22 * grad_x.y};
}
...
Vector cfd::gradient_to_physical(const JacobiMatrix& jac, Vector grad_xi) {
    ...
    return {a11 * grad_xi.x + a12 * grad_xi.y + a13 * grad_xi.z, a21 * grad_xi.x + a22 * grad_xi.y + a23 * grad_xi.z,
...
Vector cfd::gradient_to_physical_2d(const JacobiMatrix& jac, Vector grad_xi) {
    ...
    return {a11 * grad_xi.x + a12 * grad_xi.y, a21 * grad_xi.x + a22 * grad_xi.y};
}
