...
struct UmfpackSolver::Impl {
    ...
    Impl() : symbolic(nullptr), numeric(nullptr), n(0), matrix_initialized(false), factorized(false) {}
    ...
    void factorize() {
        ...
        int status = umfpack_di_symbolic(n, n, Ap.data(), Ai.data(), Ax.data(), &symbolic, control, nullptr);

        if (status != UMFPACK_OK) {
            throw std::runtime_error("UMFPACK symbolic factorization failed: " + std::to_string(status));
        }

        status = umfpack_di_numeric(Ap.data(), Ai.data(), Ax.data(), symbolic, &numeric, control, nullptr);

        if (status != UMFPACK_OK) {
            umfpack_di_free_symbolic(&symbolic);
            symbolic = nullptr;
            throw std::runtime_error("UMFPACK numeric factorization failed: " + std::to_string(status));
        }
    ...
    void solve_system(const std::vector<double>& rhs, std::vector<double>& x) {
        ...
        int status = umfpack_di_solve(UMFPACK_A, Ap.data(), Ai.data(), Ax.data(), x.data(), rhs.data(), numeric,
                                      nullptr, nullptr);
